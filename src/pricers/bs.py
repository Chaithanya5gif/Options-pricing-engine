...
def implied_vol(
    ...
    Returns:
        Optional[float]: The implied volatility, or None if convergence fails.
    """
    from src.greeks.greeks import black_scholes_vega
...
if __name__ == "__main__":
    ...
    fig.suptitle(
        "Black-Scholes Option Greeks Dashboard", fontsize=16, fontweight="bold"
    )
    ...
    axs[0, 2].plot(
        [v * 100 for v in volatilities], vegas, label="Vega", color="#059669", lw=2
    )
    ...
    axs[1, 1].plot(
        [ri * 100 for ri in rates], call_rhos, label="Call Rho", color="#0284c7", lw=2
    )
    axs[1, 1].plot(
        [ri * 100 for ri in rates], put_rhos, label="Put Rho", color="#b91c1c", lw=2
    )
