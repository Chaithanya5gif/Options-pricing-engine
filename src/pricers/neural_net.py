...
def evaluate_model(model, test_loader, device):
    ...
    ood_names = [
        "Deep OTM", "Deep ITM", "Short Expiry", "Long Expiry", 
        "Extreme Vol", "Extr. Low Spot", "Extr. High Spot"
    ]
    ...
    for name, inp in zip(ood_names, ood_inputs):
        ...
        print(f"{name:15s} -> BS: {bs_c:8.4f} | MLP: {mlp_c:8.4f} | Diff: {mlp_c - bs_c:8.4f}")
...
def test_real_market(model, device, ticker="SPY"):
    ...
    try:
        ...
        calls = calls[(calls['volume'] > 0) & (calls['impliedVolatility'] > 0.01)].copy()
        ...
        for _, row in calls.iterrows():
            ...
            inp_tensor = torch.tensor([[S_scaled, K_scaled, T, r, sigma]], dtype=torch.float32).to(device)
            with torch.no_grad():
        ...
        print(f"MLP is closer to market than BS on {len(mlp_wins)} / {len(res_df)} options.")
