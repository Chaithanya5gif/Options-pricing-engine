...
def build_stock_tree(S0: float, u: float, d: float, N: int) -> np.ndarray:
    """Build the N×(N+1) stock price lattice using vectorised NumPy.
...
def plot_convergence(
    ...
    Args:
        S0 (float, optional): Initial stock price. Defaults to 100.0.
        ...
        N_min (int, optional): Min steps. Defaults to 10.
        N_max (int, optional): Max steps. Defaults to 500.
        step (int, optional): Step size for N. Defaults to 5.
        save_path (str, optional): Path to save plot. Defaults to "crr_convergence.png".
    """
    ...
    gs = gridspec.GridSpec(
        2, 2, hspace=0.42, wspace=0.32, left=0.07, right=0.97, top=0.90, bottom=0.08
    )
    ...
    def _style_ax(ax, title, ylabel):
        ...
        ax.set_title(title, fontsize=TITLE_FS, color="white", fontweight="bold", pad=8)
    ...
    ax1.plot(
        Ns,
        eu_calls,
        color=ACCENT["eu_call"],
        lw=BIN_LW,
        label="CRR European Call",
        alpha=0.85,
    )
    ax1.axhline(
        bs_call, color=ACCENT["bs"], lw=BS_LW, ls="--", label=f"BS Call = {bs_call:.4f}"
    )
    _style_ax(ax1, "European Call — CRR convergence to Black-Scholes", "Call Price ($)")
    ...
    ax2.plot(
        ...
        color=ACCENT["eu_put"],
        lw=BIN_LW,
        label="CRR European Put",
        alpha=0.85,
    )
    ax2.axhline(
        bs_put, color=ACCENT["bs"], lw=BS_LW, ls="--", label=f"BS Put = {bs_put:.4f}"
    )
    _style_ax(ax2, "European Put — CRR convergence to Black-Scholes", "Put Price ($)")
    ...
    ax3.plot(
        Ns,
        am_calls,
        color=ACCENT["am_call"],
        lw=BIN_LW,
        label="CRR American Call",
        alpha=0.85,
    )
    ax3.plot(
        Ns,
    ...
    _style_ax(
        ax3, "American vs European Call — early exercise premium", "Call Price ($)"
    )
    ...
    _style_ax(ax4, "American vs European Put — early exercise premium", "Put Price ($)")
...
if __name__ == "__main__":
    ...
    for N_test in [50, 100, 200, 500]:
        ...
        print(
            f"    N={N_test:3d}  Call={res_c['price']:.6f} (err {err_c:.6f})"
    ...
    print(
        f"    European Call = {eu_c:.6f}  |  American Call = {am_c:.6f}"
        f"  | Early exercise premium = {am_c - eu_c:.6f}"
    )
    print(
        f"    European Put  = {eu_p:.6f}  |  American Put  = {am_p:.6f}"
