...
from black_scholes import black_scholes_delta, black_scholes_vega
...
def compute_neural_greeks(model, S, K, T, r, sigma, device):
    ...
    sigma_tensor = torch.tensor([sigma], dtype=torch.float32, requires_grad=True, device=device)
    
    # Concatenate inputs. S and sigma require grads.
    inputs = torch.cat([S_tensor, K_tensor, T_tensor, r_tensor, sigma_tensor]).unsqueeze(0)
    ...
    grads = torch.autograd.grad(price, (S_tensor, sigma_tensor), 
...
def compare_greeks(model, device):
    ...
    print(f"{'Type':<5} | {'Neural Delta':<12} | {'BS Delta':<10} | {'Neural Vega':<12} | {'BS Vega':<10}")
    print("-" * 65)
    ...
    for pt in test_points:
        ...
        # Neural
        neural_delta, neural_vega = compute_neural_greeks(model, S, K, T, r, sigma, device)
        
        print(f"{pt['name']:<5} | {neural_delta:<12.4f} | {bs_delta_call:<10.4f} | {neural_vega:<12.4f} | {bs_vega:<10.4f}")
...
def plot_delta_vs_moneyness(model, device):
    ...
    plt.plot(moneyness, neural_deltas, label='Neural Delta (orange)', color='#f97316', lw=2)
    plt.plot(moneyness, bs_deltas, label='BS Delta (blue)', color='#3b82f6', lw=2, linestyle='--')
