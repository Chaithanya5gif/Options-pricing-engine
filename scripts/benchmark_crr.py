...
print("-" * 68)
print(
    f"{'N':>6}  {'Time (ms)':>10}  {'EU Call':>10}  {'AM Put':>10}  {'BS err (call)':>14}"
)
print("-" * 68)
...
for N in N_VALUES:
    ...
    print(
        f"{N:>6}  {avg_ms:>9.2f}ms  {eu_price:>10.6f}  {am_price:>10.6f}  {err:>14.8f}"
    )
...
gs = gridspec.GridSpec(
    2, 2, hspace=0.44, wspace=0.36, left=0.07, right=0.97, top=0.90, bottom=0.08
)
...
ax1.plot(
    N_VALUES, times_ms, "o-", color="#38bdf8", lw=2, ms=6, label="Wall-clock time (ms)"
)
# Annotate key points
...
_style(
    ax1,
    "Computation Time vs N  (linear scale)",
    "Number of steps N",
    "Time per price call (ms)",
)
...
ax2.loglog(
    N_VALUES, times_ms, "o-", color="#f472b6", lw=2, ms=6, label="Wall-clock time (ms)"
)
...
on2 = scale * n_arr**2
ax2.loglog(
    N_VALUES, on2, "--", color="#facc15", lw=1.4, alpha=0.7, label="O(N²) reference"
)

_style(
    ax2,
    "Computation Time vs N  (log-log scale — O(N²) complexity)",
    "N (log scale)",
    "Time (ms, log scale)",
)
...
ax3.plot(
    N_VALUES, eu_calls, "o-", color="#34d399", lw=2, ms=6, label="CRR European Call"
)
ax3.axhline(bs_call, color="#facc15", lw=1.8, ls="--", label=f"BS Call = {bs_call:.4f}")
_style(
    ax3, "Price Accuracy vs N  (European Call)", "Number of steps N", "Call Price ($)"
)
...
ax4.semilogy(
    N_VALUES, bs_errors, "o-", color="#fb923c", lw=2, ms=6, label="|CRR − BS|  (call)"
)
ax4.axhline(
    1e-4, color="#facc15", lw=1.4, ls=":", label="4 decimal place threshold (1e-4)"
)
ax4.axhline(
    1e-2, color="#f87171", lw=1.4, ls=":", label="2 decimal place threshold (1e-2)"
)
_style(
    ax4,
    "Pricing Error vs N  (|CRR − BS|, log scale)",
    "Number of steps N",
