...
def fetch_spy_options(target_count: int = 200) -> pd.DataFrame:
    ...
    if remaining > 0:
        ...
        df_sampled = pd.concat(
            [
                df_sampled,
                leftover.sample(min(remaining, len(leftover)), random_state=42),
            ]
        )
...
def load_mlp(device) -> MLPPricer:
    ...
    model.load_state_dict(
        torch.load("mlp_pricer.pth", map_location=device, weights_only=True)
    )
    model.eval()
    print("  MLP loaded ✓")
    return model
...
def load_vae(device) -> VAE:
    ...
    model.load_state_dict(
        torch.load("vae_weights.pth", map_location=device, weights_only=True)
    )
    model.eval()
    print("  VAE loaded ✓")
    return model
...
def train_lstm_quick(device) -> tuple:
    ...
    X_train, y_train, X_test, y_test, dates_test, hist_vol_test = preprocess_data(
        df_hist
    )

    input_size = X_train.shape[2]
    # Keep LSTM on CPU — lstm_train does not move tensors to device
    lstm_cpu = LSTMVolatilityPredictor(
...
def build_vae_iv_surface(df: pd.DataFrame, vae: VAE, S: float) -> np.ndarray:
    ...
    surf_tensor = (
        torch.tensor(surface_raw.flatten(), dtype=torch.float32)
        .unsqueeze(0)
        .to(vae_device)
    )
    with torch.no_grad():
...
def vae_iv_for_option(
    smooth_surf, target_dte, target_moneyness, T_years, moneyness
) -> float:
    """
...
def price_all(
    ...
    for idx, row in df.iterrows():
        ...
        t0 = time.perf_counter()
        try:
            crr_res = price_option(
                S, K, T, r, sigma, N=200, option_type="call", exercise="european"
            )
        ...
        t0 = time.perf_counter()
        vae_sigma = vae_iv_for_option(
            smooth_surf, target_dte, target_moneyness, T, moneyness
        )
...
def compute_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for label, key in METHODS:
        ...
        mae_atm = (
            float(df.loc[atm_mask, err_col].mean()) if atm_mask.sum() > 0 else np.nan
        )
        mae_otm = (
            float(df.loc[otm_mask, err_col].mean()) if otm_mask.sum() > 0 else np.nan
        )
        ...
        print(
            f"  {label:<22}  MAE={mae:.4f}  RMSE={rmse:.4f}"
    ...
    for b in bucket_order:
        ...
        if mask.sum() == 0:
            continue
        best_label = min(METHODS, key=lambda m: df.loc[mask, f"{m[1]}_err"].mean())[0]
...
def plot_comparison_table(summary: pd.DataFrame, df: pd.DataFrame):
    ...
    ax_pct.set_title(
        "% of Options Priced Within 5% of Market",
        color=TEXT,
        fontweight="bold",
        fontsize=11,
        pad=10,
...
def main():
    ...
    print(
        f"  Smooth surface range: [{smooth_surf.min():.3f}, {smooth_surf.max():.3f}] ✓"
    )

    # ── 3. Price all options ─────────────────────────────────────────────────
    df = price_all(
        df, mlp, vae, lstm_sigma, smooth_surf, target_dte, target_moneyness, device
    )
    ...
    print(
        f"    Best %5% match : {best_pct['Method']}  ({best_pct['% within 5%']:.1f}%)"
    )
    print(
        f"    Fastest method : {summary.loc[summary['Speed ms/opt'].idxmin(), 'Method']}"
    )
    print("=" * 65 + "\n")
