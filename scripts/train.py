...
import numpy as np
from src.pricers.vae import VAE, vae_loss_function
import os
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x,) in enumerate(dataloader):
            ...
            recon_loss, kl_loss, loss = vae_loss_function(
                recon_batch, x, mu, logvar, beta=beta
            )
        ...
        if (epoch + 1) % 20 == 0:
            print(
                f"Epoch [{epoch+1}/{epochs}], Recon Loss: {avg_recon:.6f}, KL Loss: {avg_kl:.6f}"
            )
