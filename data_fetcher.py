...
def get_sp500_tickers():
    return ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'JPM', 'V', 'WMT', 'JNJ', 'PG', 'MA', 'HD', 'BAC', 'XOM', 'CVX', 'LLY', 'PFE', 'KO']
...
def fetch_iv_surface(ticker_symbol):
    ...
    surface = iv_interp.reshape(len(target_moneyness), len(target_dte)).T # Shape: (5 expiries, 10 strikes)
    return surface
...
def main():
    ...
    print(f"Found {len(tickers)} tickers. Fetching options data (this will take a while)...")
    ...
    for i, ticker in enumerate(tickers[:300]): # Limit to 300 to save time
        try:
            ...
            if i % 10 == 0:
                print(f"Processed {i}/{len(tickers[:300])}, valid surfaces so far: {len(surfaces)}")
        except Exception as e:
            pass
    ...
    if len(surfaces) < target_count:
        ...
        for _ in range(needed):
            ...
            syn_surface = base_vol + term_structure * (DTE/30.0) + skew * (M - 1.0) + convexity * (M - 1.0)**2
            # Add some noise
