...
from src.pricers.monte_carlo import mc_price, mc_antithetic, mc_control_variate
from src.pricers.binomial import price_option
from src.pricers.heston import heston_mc_price, compute_heston_smile
...
PLOTLY_LAYOUT = dict(
    ...
    hoverlabel=dict(
        bgcolor="#1c2128", bordercolor="#30363d", font=dict(color="#e6edf3")
    ),
)
DEFAULT_LEGEND = dict(
...
with st.sidebar:
    ...
    st.markdown(
        '<div class="section-header">Market Parameters</div>', unsafe_allow_html=True
    )
    ...
    T = st.slider(
        "Time to Expiry  T (years)", min_value=0.1, max_value=2.0, value=1.0, step=0.05
    )
    r = (
        st.slider("Risk-free Rate  r (%)", min_value=1, max_value=15, value=5, step=1)
        / 100.0
    )
    sigma = (
        st.slider("Volatility  σ (%)", min_value=5, max_value=80, value=20, step=1)
        / 100.0
    )

    st.markdown(
        '<div class="section-header" style="margin-top:20px;">Option Type</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="section-header" style="margin-top:20px;">Monte Carlo</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="section-header" style="margin-top:20px;">CRR Binomial</div>',
        unsafe_allow_html=True,
    )
    crr_N = st.slider("Steps  N", min_value=20, max_value=500, value=100, step=10)
    exercise_type = st.selectbox(
        "Exercise", ["european", "american"], format_func=lambda x: x.capitalize()
    )

    st.markdown(
        '<div class="section-header" style="margin-top:20px;">Heston Parameters</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown("---")
    st.markdown(
        '<div style="font-size:0.72rem; color:#8b949e; text-align:center;">Options Pricing Engine · 2026</div>',
        unsafe_allow_html=True,
    )
...
with st.spinner("Computing Monte Carlo…"):
    mc_r1, mc_r2, mc_r3 = compute_mc_all(
        float(S), float(K), T, r, sigma, n_paths, option_type
    )
...
c_delta, p_delta, gamma, vega, c_theta, p_theta, c_rho, p_rho = compute_greeks(
    S, K, T, r, sigma
)
...
moneyness = S / K
mono_label = (
    "ATM" if 0.975 <= moneyness <= 1.025 else ("ITM" if moneyness > 1.025 else "OTM")
)
...
with tab1:
    st.markdown(
        '<div class="section-header">All 6 Pricing Methods · Live Output</div>',
        unsafe_allow_html=True,
    )
    ...
    methods_info = [
        ...
        (
            ...
            mc_r2["price"],
            COLORS["anti"],
            f"SE={mc_r2['std_error']:.4f}",
        ),
        (
            "Control Variate",
            mc_r3["price"],
            COLORS["cv"],
            f"SE={mc_r3['std_error']:.4f}",
        ),
    ...
    fig_bar.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(
            text="Option Price by Method", font=dict(size=15, color="#e6edf3"), x=0
        ),
        xaxis_title="Method",
    ...
    # ── MC Convergence ──────────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Monte Carlo Convergence  (Basic vs Antithetic vs Control Variate)</div>',
        unsafe_allow_html=True,
    )
    ...
    def mc_convergence_data(S, K, T, r, sigma, option_type):
        ...
        for i, n in enumerate(Ns):
            ...
            a_p.append(ra["price"])
            c_p.append(rc["price"])
            b_se.append(rb["std_error"])
            a_se.append(ra["std_error"])
            c_se.append(rc["std_error"])
    ...
    for name, prices, color, dash in [
        ...
        fig_conv.add_trace(
            go.Scatter(
                ...
                marker=dict(size=6),
                hovertemplate=f"<b>{name}</b><br>N=%{{x:,}}<br>Price=$%{{y:.5f}}<extra></extra>",
            ),
            row=1,
            col=1,
        )
    ...
    # ── Summary table ───────────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Summary Table</div>', unsafe_allow_html=True
    )
    summary_data = {
        "Method": [
            "Black-Scholes",
        ...
        "Std Error": [
            "—",
            "—",
            f"{mc_r1['std_error']:.5f}",
            f"{mc_r2['std_error']:.5f}",
            f"{mc_r3['std_error']:.5f}",
            f"{h_se:.5f}",
        ],
        "Speed": [
            "< 1 ms",
            f"{crr_ms:.1f} ms",
            f"{mc_r1['elapsed_ms']:.1f} ms",
            f"{mc_r2['elapsed_ms']:.1f} ms",
            f"{mc_r3['elapsed_ms']:.1f} ms",
            f"{h_ms:.1f} ms",
        ],
        "Type": [
            "Analytical",
...
with tab2:
    st.markdown(
        '<div class="section-header">Black-Scholes Greeks at Current Parameters</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_g2:
        greek_rows = [
            (
                "Δ Delta (Call)",
                f"{c_delta:.5f}",
                "Sensitivity to spot price — fraction of underlying",
            ),
            ...
            (
                "Θ Theta (Call)",
                f"{c_theta:.5f}",
                "Time decay per year; always negative for calls",
            ),
        ...
        for name, val, interp in greek_rows:
            st.markdown(
                f"<tr><td>{name}</td><td class='greek-val'>{val}</td><td style='color:#8b949e'>{interp}</td></tr>",
                unsafe_allow_html=True,
            )
    ...
    fig_greeks.add_trace(
        go.Scatter(
            ...
            hovertemplate="S=%{x:.1f}<br>Delta=%{y:.4f}<extra>Call</extra>",
        ),
        row=1,
        col=1,
    )
    fig_greeks.add_trace(
        go.Scatter(
            ...
            hovertemplate="S=%{x:.1f}<br>Delta=%{y:.4f}<extra>Put</extra>",
        ),
        row=1,
        col=1,
    )
    ...
    fig_greeks.add_trace(
        go.Scatter(
            ...
            showlegend=False,
            hovertemplate="S=%{x:.1f}<br>Gamma=%{y:.5f}<extra></extra>",
        ),
        row=1,
        col=2,
    )
    ...
    fig_greeks.add_trace(
        go.Scatter(
            ...
            showlegend=False,
            hovertemplate="S=%{x:.1f}<br>Vega=%{y:.4f}<extra></extra>",
        ),
        row=2,
        col=1,
    )
    ...
    fig_greeks.add_trace(
        go.Scatter(
            ...
            showlegend=False,
            hovertemplate="S=%{x:.1f}<br>Theta=%{y:.4f}<extra>Call</extra>",
        ),
        row=2,
        col=2,
    )
    fig_greeks.add_trace(
        go.Scatter(
            ...
            showlegend=False,
            hovertemplate="S=%{x:.1f}<br>Theta=%{y:.4f}<extra>Put</extra>",
        ),
        row=2,
        col=2,
    )
    ...
    for row, col in [(1, 1), (1, 2), (2, 1), (2, 2)]:
        fig_greeks.add_vline(
            x=K, line_dash="dot", line_color="#facc15", line_width=1.2, row=row, col=col
        )
    ...
    fig_greeks.update_layout(
        **PLOTLY_LAYOUT,
        height=560,
        title=dict(
            text=f"BS Greeks vs Spot  [K={K}, T={T:.2f}yr, r={r:.1%}, σ={sigma:.1%}]",
    ...
    # ── Vega vs σ ────────────────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Vega vs Volatility  &  Delta vs Time to Expiry</div>',
        unsafe_allow_html=True,
    )
...
with tab3:
    st.markdown(
        '<div class="section-header">Heston Stochastic Volatility — Implied Vol Smile vs Flat BS</div>',
        unsafe_allow_html=True,
    )
    ...
    n_smile_paths = st.select_slider(
        "Smile Paths (more = smoother, slower)",
    ...
    # Filter valid
    valid = [
        (strikes_smile[i], h_ivs[i], bs_ivs[i])
        for i in range(len(h_ivs))
        if h_ivs[i] is not None
    ]
    if valid:
        vK, vh, vbs = zip(*valid)
        ...
        fig_smile.update_xaxes(
            gridcolor="#30363d",
            linecolor="#30363d",
            tickfont=dict(color="#8b949e", size=10),
        )
        fig_smile.update_yaxes(
            gridcolor="#30363d",
            linecolor="#30363d",
            tickfont=dict(color="#8b949e", size=10),
        )
        ...
        # ── ATM vs wing stats ──────────────────────────────────────────────
        atm_idx = min(
            range(len(moneyness_arr)), key=lambda i: abs(moneyness_arr[i] - 1.0)
        )
        ...
        c1.metric(
            "ATM IV (Heston)",
            f"{atm_iv:.2f}%",
            delta=f"{atm_iv - math.sqrt(h_theta)*100:+.2f}% vs flat BS",
        )
        c2.metric(
            "OTM Put Wing IV",
            f"{wing_iv:.2f}%",
            delta=f"K/S={moneyness_arr[wing_idx]:.2f}",
        )
        c3.metric(
            "Skew (Wing − ATM)",
            f"{skew:+.2f}%",
            delta="left skew from ρ<0" if h_rho < 0 else "right skew",
        )
        c4.metric(
            "BS Flat IV",
            f"{math.sqrt(h_theta)*100:.1f}%",
            delta="constant across strikes",
        )
    else:
        st.warning(
            "Could not extract implied vols for these parameters. Try lower σᵥ or fewer paths."
        )

    # ── IV sensitivity ─────────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Smile Shape vs ρ (spot-vol correlation)</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.spinner("Computing ρ sensitivity…"):
        rho_data = rho_sensitivity(
            float(S), T, r, h_v0, h_kappa, h_theta, h_sigma_v, rho_paths
        )
...
with tab4:
    st.markdown(
        '<div class="section-header">SPY Test-Set Results  (200 options · Market Prices vs Model Prices)</div>',
        unsafe_allow_html=True,
    )
    ...
    def highlight_table(row):
        styles = [""] * len(row)
        mae_col = (
            df_cmp.columns.get_loc("MAE (all)")
            if "MAE (all)" in df_cmp.columns
            else None
        )
        pct_col = (
            df_cmp.columns.get_loc("% within 5%")
        ...
        if mae_col is not None:
            ...
            if mae_val == min_mae:
                styles[mae_col] = (
                    "background-color: rgba(52,211,153,0.15); color: #34d399; font-weight:600;"
                )
            elif mae_val == df_cmp["MAE (all)"].max():
                styles[mae_col] = (
                    "background-color: rgba(248,113,113,0.12); color: #f87171;"
                )
    ...
    styled = (
        df_cmp.style.apply(highlight_table, axis=1)
    ...
    with col_ch1:
        fig_mae = go.Figure()
        mae_col_name = (
            "MAE (all)" if "MAE (all)" in df_cmp.columns else df_cmp.columns[1]
        )
        bar_colors = [
            COLORS["bs"],
            COLORS["crr"],
    ...
    # ── Radar chart ─────────────────────────────────────────────────────────
    st.markdown(
        '<div class="section-header">Radar — Multi-Metric Model Profile</div>',
        unsafe_allow_html=True,
    )

    radar_methods = df_cmp["Method"].tolist()
    if all(
        c in df_cmp.columns for c in ["MAE (all)", "RMSE", "% within 5%", "MAE ATM"]
    ):
        ...
        max_speed = (
            df_cmp["Speed ms/opt"].max() if "Speed ms/opt" in df_cmp.columns else 1
        )
        ...
        fig_radar.update_layout(
            **PLOTLY_LAYOUT,
            ...
            title=dict(
                text="Normalised Model Performance (higher = better)",
                font=dict(size=14, color="#e6edf3"),
                x=0,
            ),
            height=480,
    ...
    if "Speed ms/opt" in df_cmp.columns:
        st.markdown(
            '<div class="section-header">Speed vs Accuracy Trade-off</div>',
            unsafe_allow_html=True,
        )
