...
def mc_price(
    ...
    Returns
    -------
    dict  price, std_error, n_paths, elapsed_ms
    """
...
def mc_control_variate(
    ...
    which is typically the dominant noise source.
    """
    rng = np.random.default_rng(seed)
    t0  = time.perf_counter()
...
def plot_mc_analysis(
    ...
    basic_prices   = [mc_price(S0, K, T, r, sigma, n, seed=i)["price"]      for i, n in enumerate(Ns)]
    anti_prices    = [mc_antithetic(S0, K, T, r, sigma, n, seed=i)["price"] for i, n in enumerate(Ns)]
    cv_prices      = [mc_control_variate(S0, K, T, r, sigma, n, seed=i)["price"] for i, n in enumerate(Ns)]

    basic_ses      = [mc_price(S0, K, T, r, sigma, n, seed=i)["std_error"]      for i, n in enumerate(Ns)]
    anti_ses       = [mc_antithetic(S0, K, T, r, sigma, n, seed=i)["std_error"] for i, n in enumerate(Ns)]
    cv_ses         = [mc_control_variate(S0, K, T, r, sigma, n, seed=i)["std_error"] for i, n in enumerate(Ns)]

    # --- Sampling distribution (many independent runs at N=10000) ---
    N_REPS = 500
    basic_rep = [mc_price(S0, K, T, r, sigma, 10_000, seed=s)["price"]      for s in range(N_REPS)]
    anti_rep  = [mc_antithetic(S0, K, T, r, sigma, 10_000, seed=s)["price"] for s in range(N_REPS)]
    ...
    def _style(ax, title, xlabel, ylabel, legend_loc="upper right"):
        ...
        ax.legend(fontsize=8, framealpha=0.25, loc=legend_loc,
                  labelcolor="white", facecolor="#21262d")

    # Panel 1: Price convergence
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.semilogx(Ns, basic_prices, "o-", color=COLORS["basic"], lw=1.8, ms=5, label="Basic MC")
    ax1.semilogx(Ns, anti_prices,  "s-", color=COLORS["anti"],  lw=1.8, ms=5, label="Antithetic")
    ax1.semilogx(Ns, cv_prices,    "^-", color=COLORS["cv"],    lw=1.8, ms=5, label="Control Variate")
    ax1.axhline(bs_call, color=COLORS["bs"], lw=1.6, ls="--", label=f"BS = {bs_call:.4f}")
    _style(ax1, "Price Convergence vs N  (log scale)", "N paths (log)", "Call Price ($)", "lower right")

    # Panel 2: Std Error vs N (log-log)
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.loglog(Ns, basic_ses, "o-", color=COLORS["basic"], lw=1.8, ms=5, label="Basic MC SE")
    ax2.loglog(Ns, anti_ses,  "s-", color=COLORS["anti"],  lw=1.8, ms=5, label="Antithetic SE")
    ax2.loglog(Ns, cv_ses,    "^-", color=COLORS["cv"],    lw=1.8, ms=5, label="Control Variate SE")
    ...
    _style(ax2, "Standard Error vs N  (log-log — 1/√N decay)", "N paths (log)", "Std Error (log)")
    ...
    ax3.semilogx(Ns, ratio_anti, "s-", color=COLORS["anti"], lw=1.8, ms=5,
                 label="SE(Basic) / SE(Antithetic)")
    ax3.semilogx(Ns, ratio_cv,   "^-", color=COLORS["cv"],   lw=1.8, ms=5,
                 label="SE(Basic) / SE(Control Variate)")
    ax3.axhline(1.0, color="#6b7280", lw=1.2, ls=":", label="No improvement (ratio=1)")
    ax3.axhline(math.sqrt(2), color=COLORS["anti"], lw=1.0, ls=":",
                label=f"Theoretical antithetic ~√2 ≈ {math.sqrt(2):.2f}")
    _style(ax3, "Variance Reduction Ratio  (>1 = improvement)", "N paths (log)", "SE Ratio")

    # Panel 4: Sampling distribution — basic vs antithetic at N=10000
    ax4 = fig.add_subplot(gs[1, 1])
    ax4.hist(basic_rep, bins=40, alpha=0.55, color=COLORS["basic"], label="Basic MC (N=10k)",
             density=True, edgecolor="none")
    ax4.hist(anti_rep,  bins=40, alpha=0.55, color=COLORS["anti"],  label="Antithetic (N=10k)",
             density=True, edgecolor="none")
    ax4.axvline(bs_call, color=COLORS["bs"], lw=2, ls="--", label=f"True BS = {bs_call:.4f}")
    _style(ax4,
           f"Sampling Distribution  (500 independent runs, N=10k)\n"
           f"Basic σ={np.std(basic_rep):.4f}  |  Antithetic σ={np.std(anti_rep):.4f}",
           "Estimated Call Price", "Density", "upper right")
...
if __name__ == "__main__":
    ...
    print(f"    Price = {res['price']:.6f}   SE = {res['std_error']:.6f}   "
    ...
    print(f"    Price = {res_a['price']:.6f}   SE = {res_a['std_error']:.6f}   "
          f"95% CI ± {1.96*res_a['std_error']:.6f}   BS err = {abs(res_a['price']-bs_call):.6f}")
    print(f"    Variance reduction vs basic: {(res['std_error']/res_a['std_error']):.2f}× "
          f"(theoretical ≈ √2 ≈ 1.41×)")

    # ── [3] Control Variate
    res_cv = mc_control_variate(S0, K, T, r, sigma, 100_000)
    print(f"\n[3] Control Variate  (N=100,000,  β={res_cv['beta']:.4f})")
    print(f"    Price = {res_cv['price']:.6f}   SE = {res_cv['std_error']:.6f}   "
          f"95% CI ± {1.96*res_cv['std_error']:.6f}   BS err = {abs(res_cv['price']-bs_call):.6f}")
    print(f"    Variance reduction vs basic: {(res['std_error']/res_cv['std_error']):.2f}×")

    # ── [4] Comparison Table
    print(f"\n[4] RESULTS TABLE — MC vs Antithetic vs Control Variate")
    print("-" * 72)
    print(f"{'Method':<18} {'N':>8} {'Price':>10} {'Std Err':>10} {'95%CI±':>10} {'BSErr':>10} {'ms':>8}")
    print("-" * 72)

    rows, bs_ref = build_comparison_table(S0, K, T, r, sigma,
    ...
    for row in rows:
        if row["N paths"] != prev_n and prev_n is not None:
            print()
        prev_n = row["N paths"]
        print(f"  {row['Method']:<16} {row['N paths']:>8,} "
